...
class Metronome:
    ...
    def interface(self):
        ...
        button_stop = Button(
            frame, text="Stop", width=10, height=2, command=lambda: self.stop_counter()
        )
    ...
    def counter(self, spinbox):
        ...
        if self.start:
            ...
            self.bpm += (
                5  # this is the intervension happened. tame this and make a good one
            )

            if self.beat == 6:  # 6/8 time
                self.time = int((60 / (self.bpm / 0.5) - 0.1) * 1000)
            else:
